...
public class FrmAddGoods extends JDialog implements ActionListener{
	...
	public FrmAddGoods(Dialog d, String s, boolean b , BeanStore Store){
		super(d,s,b);
		BS=Store;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==this.btnCancel) {
			...
			if(name.equals("")||category.equals("")){
				JOptionPane.showMessageDialog(null, "商品名称和商品类别不得为空","错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			if(edtPrice.equals("")||edtSPrice.equals("")){
				JOptionPane.showMessageDialog(null, "商品价格和优惠价格得为空","错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			try{
				price = Double.parseDouble(this.edtPrice.getText());
			}catch(Exception ex){
				JOptionPane.showMessageDialog(null, this.edtPrice.getText()+"不是一个合法的整数","错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			try{
				sprice = Double.parseDouble(this.edtSPrice.getText());
			}catch(Exception ex){
				JOptionPane.showMessageDialog(null, this.edtSPrice.getText()+"不是一个合法的整数","错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			try {
				TakeawayUtil.GoodsManager.addGoods(this.Goods);
				this.setVisible(false);
			} catch (Exception e1) {
				this.Goods=null;
