...
public class FrmModifyStore extends JDialog implements ActionListener{
	...
	public FrmModifyStore(Dialog d, String s, boolean b,BeanStore BS){
		super(d,s,b);
		Store = BS;
		...
		this.edtName.setText(BS.getName());
		workPane.add(edtName);
		workPane.add(labelRating);
		this.edtRating.setText(BS.getRating()+"");
		workPane.add(edtRating);
		workPane.add(labelPCC);
		this.edtPCC.setText(BS.getPCC()+"");
		workPane.add(edtPCC);
		...
		// 屏幕居中显示
				...
				this.addWindowListener(new WindowAdapter() {
					public void windowClosing(WindowEvent e) {
						FrmModifyStore.this.Store=null;
					}
				});
	}
	@Override
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==this.btnCancel) {
			...
			try {
				(new ExampleStoreManager()).modifyStore(this.Store);
				this.setVisible(false);
			} catch (Exception e1) {
				this.Store=null;
